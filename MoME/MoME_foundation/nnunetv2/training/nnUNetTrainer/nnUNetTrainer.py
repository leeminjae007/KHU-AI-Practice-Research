...
class nnUNetTrainer(object):
    ...
    def train_step(self, batch: dict,r) -> dict:
        data = batch['data']
        target = batch['target']
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            Feature1,output1 = self.network1(data)
            Feature2,output2 = self.network2(data)
            Feature3,output3 = self.network3(data)
            ...
            Final_output = [output1[i]*output[i][:,:2,:,:,:]+output2[i]*output[i][:,2:4,:,:,:]+output3[i]*output[i][:,4:6,:,:,:] \
                +output4[i]*output[i][:,6:8,:,:,:] +output5[i]*output[i][:,8:10,:,:,:] for i in range(len(output))]
            output = Final_output
            # # del data
            l1 = 0
            outputs = [output1, output2, output3, output4, output5]
            for num, i in enumerate(modkey):
                sub_target = [t[num:num+1] for t in target]
                if i >= 0 and i < len(outputs):
                    # l1 += self.loss(outputs[i][num, ...], target[num, ...])
                    sub_output = [o[num:num+1] for o in outputs[i]]
                else:
                    # Handle the case when i is out of bounds or negative
                    # l1 += self.loss(output5[num, ...], target[num, ...])
                    sub_output = [o[num:num+1] for o in output5]
                l1 += self.loss(sub_output, sub_target)
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            # coutput = torch.cat((output1[0], output2[0]), dim=1)
            # output = self.network(coutput)
            # Final_output = [output1[i]*output[i][:,:3,:,:,:]+output2[i]*output[i][:,3::,:,:,:] for i in range(len(output))]
            # output = Final_output
            # del data
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        global_dc_per_class = [i for i in [2 * i / (2 * i + j + k+1e-10) for i, j, k in
                                           zip(tp, fp, fn)]]
    ...
    def save_checkpoint(self, filename: str) -> None:
        if self.local_rank == 0:
            if not self.disable_checkpointing:
                ...
                torch.save(checkpoint, filename.replace('best','best1').replace('latest','latest1').replace('final','final1'))    
                ...
                torch.save(checkpoint, filename.replace('best','best2').replace('latest','latest2').replace('final','final2'))         
                ...
                torch.save(checkpoint, filename.replace('best','best3').replace('latest','latest3').replace('final','final3'))    
                ...
                torch.save(checkpoint, filename.replace('best','best4').replace('latest','latest4').replace('final','final4'))      
                ...
                torch.save(checkpoint, filename.replace('best','best5').replace('latest','latest5').replace('final','final5'))                   
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            for batch_id in range(self.num_iterations_per_epoch):
                train_outputs.append(self.train_step(next(self.dataloader_train),r))
